...
class TestAdapterBehavior(unittest.TestCase):
    ...
    def test_add_ack_reaction_uses_ok_emoji(self):
        ...
        self.assertEqual(captured["request"].request_body.reaction_type["emoji_type"], "OnIt")

    @patch.dict(os.environ, {}, clear=True)
    @unittest.skipUnless(_HAS_LARK_OAPI, "lark-oapi not installed")
    def test_on_processing_complete_swaps_ack_for_done(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.base import ProcessingOutcome
        from gateway.platforms.feishu import FeishuAdapter
        ...
        class _ReactionAPI:
            def create(self, request):
                ...
                return SimpleNamespace(
                    success=lambda: True,
                    data=SimpleNamespace(reaction_id="r_done"),
                )
        ...
        adapter._ack_reaction_ids["om_msg"] = "r_ack"
        ...
        self.assertEqual(deleted[0].message_id, "om_msg")
        self.assertEqual(deleted[0].reaction_id, "r_ack")
        self.assertEqual(created[0].request_body.reaction_type["emoji_type"], "DONE")
        self.assertNotIn("om_msg", adapter._ack_reaction_ids)

    @patch.dict(os.environ, {}, clear=True)
    @unittest.skipUnless(_HAS_LARK_OAPI, "lark-oapi not installed")
    def test_on_processing_complete_skips_done_for_failures(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.base import ProcessingOutcome
        from gateway.platforms.feishu import FeishuAdapter

        adapter = FeishuAdapter(PlatformConfig())
        reaction_api = SimpleNamespace(create=Mock(), delete=Mock())
        adapter._client = SimpleNamespace(
        ...
        reaction_api.create.assert_not_called()
        reaction_api.delete.assert_not_called()
        self.assertNotIn("om_msg", adapter._ack_reaction_ids)
    ...
    def test_add_ack_reaction_logs_warning_on_failure(self):
        ...
        self.assertTrue(
            any("Failed to add ack reaction (OnIt) to om_msg" in entry for entry in logs.output),
            logs.output,
        )
    ...
    def test_user_reaction_with_managed_emoji_is_still_routed(self):
        ...
        event = SimpleNamespace(
            message_id="om_msg",
            operator_type="user",
            reaction_type=SimpleNamespace(emoji_type="OnIt"),
        )
        data = SimpleNamespace(event=event)
