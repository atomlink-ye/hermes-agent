...
_FEISHU_MESSAGE_STYLE_KEY = "_feishu_message_style"
_FEISHU_INTERACTIVE_CARD_STYLE = "interactive_card"
# ---------------------------------------------------------------------------
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._message_style_cache: Dict[str, str] = {}
    ...
    async def send(
        ...
        try:
            for chunk in chunks:
                ...
                try:
                    ...
                    if msg_type == "interactive":
                        logger.warning("[Feishu] Interactive card send failed; falling back to standard message")
                ...
                if msg_type == "interactive" and not self._response_succeeded(response):
                    logger.warning("[Feishu] Interactive card rejected by API response; falling back to standard message")
                    msg_type, payload = self._build_outbound_payload(chunk)
                    response = await self._feishu_send_with_retry(
                        chat_id=chat_id,
                        msg_type=msg_type,
            ...
            if result.success and result.message_id:
                self._message_style_cache[str(result.message_id)] = msg_type
            return result
        except Exception as exc:
    ...
    async def edit_message(
        ...
        *,
        ...
        try:
            msg_type, payload = self._build_outbound_payload(content, message_id=message_id)
            body = self._build_update_message_body(msg_type=msg_type, content=payload)
            request = self._build_update_message_request(message_id=message_id, request_body=body)
            response = await asyncio.to_thread(self._client.im.v1.message.update, request)
            result = self._finalize_send_result(response, "update failed")
            if not result.success and msg_type == "interactive":
                logger.warning("[Feishu] Interactive card update failed; falling back to standard message")
            ...
            if result.success:
                ...
                self._message_style_cache[str(message_id)] = msg_type
            return result
        except Exception as exc:
    ...
    async def on_processing_complete(self, event: MessageEvent, outcome: ProcessingOutcome) -> None:
        """Swap the receipt ACK for a DONE reaction after successful processing."""
        message_id = event.message_id
        if not message_id:
            return
        ack_reaction_id = self._ack_reaction_ids.pop(message_id, None)
        if outcome != ProcessingOutcome.SUCCESS:
            return
        if ack_reaction_id:
            await self._delete_reaction(message_id, ack_reaction_id, label="ack")
        await self._add_done_reaction(message_id)
    ...
    async def _handle_message_with_guards(self, event: MessageEvent) -> None:
        ...
        - ACK indicator: adds an OnIt reaction to the triggering message before handing
          off to the agent; the completion hook swaps it to DONE on success.
        """
    ...
    async def _add_ack_reaction(self, message_id: str) -> Optional[str]:
        ...
        reaction_id = await self._create_reaction(message_id, _FEISHU_ACK_EMOJI, label="ack")
        if reaction_id:
            self._ack_reaction_ids[message_id] = reaction_id
        return reaction_id

    async def _add_done_reaction(self, message_id: str) -> Optional[str]:
        """Add a DONE emoji reaction to signal the request completed successfully."""
        return await self._create_reaction(message_id, _FEISHU_DONE_EMOJI, label="done")

    async def _create_reaction(self, message_id: str, emoji_type: str, *, label: str) -> Optional[str]:
        """Create a Feishu message reaction and return its reaction ID when available."""
        ...
        try:
            ...
            logger.warning(
                "[Feishu] Failed to add %s reaction (%s) to %s: code=%s msg=%s",
                label,
                emoji_type,
                message_id,
            ...
            logger.warning(
                "[Feishu] Failed to add %s reaction (%s) to %s",
                label,
                emoji_type,
                message_id,
                exc_info=True,
    ...
    async def _delete_reaction(self, message_id: str, reaction_id: str, *, label: str) -> bool:
        """Delete a Feishu message reaction by reaction ID."""
        ...
        try:
            ...
            if response and getattr(response, "success", lambda: False)():
                return True
            logger.warning(
                "[Feishu] Failed to delete %s reaction %s from %s: code=%s msg=%s",
                label,
                reaction_id,
                message_id,
                getattr(response, "code", None),
            ...
            logger.warning(
                "[Feishu] Failed to delete %s reaction %s from %s",
                label,
                reaction_id,
                message_id,
                exc_info=True,
    ...
    def _build_outbound_payload(
        ...
        *,
        ...
        requested_style = str((metadata or {}).get(_FEISHU_MESSAGE_STYLE_KEY, "") or "").strip().lower()
        cached_style = str(self._message_style_cache.get(message_id, "") or "").strip().lower() if message_id else ""
        if requested_style == _FEISHU_INTERACTIVE_CARD_STYLE or cached_style == "interactive":
